...
async def get_all_orders(token: str = Depends(oauth2_scheme)):
    ...
    try:
        await cursor.execute("""
            ...
            FROM Orders o
                LEFT JOIN DeliveryInfo di ON o.UserName = di.FirstName
        ...
        for row in orders_data:
            ...
            # FIXED: Select 'ProductCategory' from the OrderItems table
            await cursor.execute("""
                SELECT ProductName, Quantity, Price, ProductCategory 
                FROM OrderItems 
                WHERE OrderID = ?
            """, (order_id,))
            ...
            for item in item_rows:
                item_list.append({
                    ...
                    "quantity": item[1],
                    "price": float(item[2]),
                    "category": item[3]  # FIXED: Add the category to the item details
                })
...
async def add_to_cart(item: CartItem, token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        product_category = item.product_category or 'General'
        ...
        if existing_item:
            ...
            """, (
                ...
                item.product_name,
                product_type,
                product_category,  # Now guaranteed to have a value
                item.quantity,
                item.price
            ))
