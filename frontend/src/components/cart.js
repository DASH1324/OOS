...
const Cart = () => {
  ...
  const handleCheckboxChange = (item, checked) => {
    if (checked) {
      // Ensure we preserve the original category when selecting items
      setSelectedCartItems(prev => [...prev, {
        ...item,
        // Keep the original product category, not any modified version
  ...
  const handleSelectAllChange = (checked) => {
    if (checked) {
      // Preserve original categories for all items
      const itemsWithOriginalCategories = cartItems.map(item => ({
        ...item,
        ProductCategory: item.ProductCategory || item.product_category || item.category
      }));
      setSelectedCartItems(itemsWithOriginalCategories);
    } else {
  ...
  const handleIncrement = (index) => {
    ...
    // Update quantity in selectedCartItems if item is selected, preserve category
  ...
  const handleCheckoutClick = (e) => {
    ...
    // Ensure all selected items have their original categories preserved
    const itemsWithPreservedCategories = selectedCartItems.map(item => ({
      ...item,
      // Make sure we pass the original category to checkout
      category: item.ProductCategory || item.product_category || item.category,
      // Also keep it in the expected format
      ProductCategory: item.ProductCategory || item.product_category || item.category,
      product_category: item.ProductCategory || item.product_category || item.category
    }));

    console.log("🛒 Checkout Items with Categories:", itemsWithPreservedCategories);

    navigate('/checkout', {
      state: {
        cartItems: itemsWithPreservedCategories,
                  ...
                  {cartItems.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ verticalAlign: 'middle' }}>
                        {/* Display the original category */}
                        {item.ProductCategory || item.product_category || item.category || '-'}
                      </td>
                      <td style={{ textAlign: 'center' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white' }}>
                ...
                <tfoot>
                  ...
                  <tr className="total-row">
                    ...
                    <td></td>
                    <td style={{ textAlign: 'right', padding: '8px', fontWeight: 'bold', verticalAlign: 'middle' }}>
                      ₱{(selectedCartItems.reduce((acc, item) => acc + item.ProductPrice * item.quantity, 0) + (orderTypeMain === 'Delivery' ? 50 : 0)).toFixed(2)}
                    </td>
                  </tr>
