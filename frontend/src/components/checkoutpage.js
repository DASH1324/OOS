...
const CheckoutPage = () => {
  ...
  // FIXED: Ensure every item has a category with proper fallback
  const cartPayload = cartItems.map(item => {
    const finalCategory = item.ProductCategory || item.product_category || item.category || 'Online';
    
    console.log(`Item: ${item.ProductName}, Final Category: ${finalCategory}`);
    
    return {
      product_id: item.product_id,
      product_name: item.ProductName,
      product_type: item.ProductType || '',
      product_category: finalCategory, // Guaranteed to have a value
